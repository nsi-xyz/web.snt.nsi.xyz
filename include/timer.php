<?php
if ($session->currentUserIsPlayer()) {
  $sessionsTimes[$session->getCurrentUser()->getGameSessionId()] = $session->getCurrentUser()->getGameSession()->getTimeLeft();
}
?>
<?php if (isset($sessionsTimes)) : ?>
  <script>
  const sessionTimers = <?= json_encode($sessionsTimes) ?>;
